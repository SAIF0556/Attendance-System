...
class AttendanceSystem:
    def __init__(self, root):
        ...
        self.root.configure(bg="#071952")  # Deep blue color
    ...
    def setup_gui(self):
        # Main Frame
        main_frame = tk.Frame(self.root, bg="#071952", padx=20, pady=20)
        main_frame.pack(fill="both", expand=True)

        # Load the image and keep a reference
        self.logo_img = tk.PhotoImage(file="./image.png")
        logo_label = tk.Label(main_frame, image=self.logo_img, bg="#071952",height=95)
        logo_label.grid(row=0, column=1, padx=10, sticky="w")

        # Teacher's Info Frame
        teacher_frame = tk.LabelFrame(main_frame, text="Teacher Info", bg="#071952", fg="white", padx=20, pady=20)
        teacher_frame.grid(row=1, column=0, columnspan=2, padx=20, pady=20, sticky="ew")

        # Teacher's Info Inputs
        tk.Label(teacher_frame, text="Teacher's Name:", bg="#071952", fg="white").grid(row=1, column=0, sticky="w", pady=10, padx=10)
        tk.Entry(teacher_frame, textvariable=self.teacher_name, width=120, bg="white").grid(row=2, column=0, pady=10, padx=10)

        tk.Label(teacher_frame, text="Degree:", bg="#071952", fg="white").grid(row=1, column=1, sticky="w", pady=10, padx=10)
        tk.Entry(teacher_frame, textvariable=self.degree, width=120, bg="white").grid(row=2, column=1, pady=10, padx=10)

        tk.Label(teacher_frame, text="Semester:", bg="#071952", fg="white").grid(row=3, column=0, sticky="w", pady=10, padx=10)
        tk.Entry(teacher_frame, textvariable=self.semester, width=120, bg="white").grid(row=4, column=0, pady=10, padx=10)

        tk.Label(teacher_frame, text="Course Name:", bg="#071952", fg="white").grid(row=3, column=1, sticky="w", pady=10, padx=10)
        tk.Entry(teacher_frame, textvariable=self.course_name, width=120, bg="white").grid(row=4, column=1, pady=10, padx=10)

        tk.Label(teacher_frame, text="Year:", bg="#071952", fg="white").grid(row=5, column=0, sticky="w", pady=10, padx=10)
        tk.Entry(teacher_frame, textvariable=self.year, width=120, bg="white").grid(row=6, column=0, pady=10, padx=10)

        tk.Label(teacher_frame, text="Date:", bg="#071952", fg="white").grid(row=5, column=1, sticky="w", pady=10, padx=10)
        tk.Entry(teacher_frame, textvariable=self.date, width=120, bg="white").grid(row=6, column=1, pady=10, padx=10)
        # Student Info Header
        student_header_frame = tk.Frame(main_frame, bg="#071952")
        ...
        label_style = {'bg': "#071952", 'fg': "white", 'width': 70, 'highlightbackground': 'white', 'highlightthickness': 1}
        ...
        # Attendance List
        self.attendance_frame = tk.Frame(main_frame, bg="#071952", width=140)
        ...
        tk.Button(main_frame, text="Load Students", command=self.load_students_from_file, bg="white", fg="#071952").grid(row=5, column=0, columnspan=2, pady=10)

        # Save Attendance
        tk.Button(main_frame, text="Save Attendance", command=self.save_attendance, bg="white", fg="#071952").grid(row=6, column=0, columnspan=2, pady=10)
    ...
    def save_attendance(self):
        ...
        filename = f"{current_month_folder}/attendance_{datetime.now().strftime('%Y-%m-%d')}.csv"
        ...
        with open(filename, mode='w', newline='') as file:  # Open the file in write mode to replace existing data
            ...
            writer.writerow([self.teacher_name.get(), self.degree.get(), self.semester.get(), self.course_name.get(), self.year.get(), datetime.now().strftime("%Y-%m-%d"), datetime.now().strftime("%H:%M:%S")])
